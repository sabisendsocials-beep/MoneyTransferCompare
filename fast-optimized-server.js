/**
 * Fast and optimized server for the rate comparison application
 * This server prioritizes quick startup while maintaining all features
 */
...
app.post("/api/direct-verify", async (req, res) => {
  try {
    ...
    try {
      ...
      console.error(`SQL error in verification: ${sqlError}`);
      
      // Fallback to ORM approach if SQL approach fails
      try {
        ...
        return res.json({
          success: true,
          message: `Rate ${verified ? 'verified' : 'unverified'} successfully (using ORM)`,
          rate: updatedRate
        });
      } catch (ormError) {
...
app.get("/api/verified-rates", async (req, res) => {
  try {
    const verifiedRates = await db
      ...
      .where(eq(schema.exchangeRates.verified, true))
...
// Add endpoint to manually trigger rate updates
app.post("/api/manual-update", async (req, res) => {
  const { updateType } = req.body;
  
  try {
    if (updateType === 'rates') {
      // Schedule rate update in the background
      setTimeout(async () => {
        try {
          const wiseModule = await import('./server/api/wiseApi.js');
          await wiseModule.default();
          log("Wise API rates updated manually");
        } catch (error) {
          log(`Error in manual rate update: ${error}`);
        }
      }, 100);
    ...
    else if (updateType === 'trustpilot') {
      // Schedule TrustPilot update in the background
      setTimeout(async () => {
        try {
          const { updateTrustpilotRatings } = await import('./server/trustpilotRatings.js');
          await updateTrustpilotRatings();
          log("TrustPilot ratings updated manually");
        } catch (error) {
          log(`Error in manual TrustPilot update: ${error}`);
        }
      }, 100);
      
      return res.json({
        success: true,
        message: 'TrustPilot update has been triggered in the background'
      });
    }
    else {
      return res.status(400).json({
        success: false,
        message: 'Invalid update type. Use "rates" or "trustpilot"'
      });
    }
  } catch (error) {
...
// Schedule tasks at a specific hour
...
(async () => {
  ...
  setTimeout(async () => {
    try {
      ...
      // Schedule rate updates 3 times per day (6 AM, 2 PM, 10 PM)
