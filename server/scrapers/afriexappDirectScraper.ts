/**
 * Afriexapp Direct Scraper
 * 
 * This file implements a direct scraping solution for Afriexapp
...
/**
 * Scrape Afriexapp website for exchange rates
 * Uses ONLY the CSS selector identified from the HTML with no fallbacks
 */
export async function scrapeAfriexappDirect(
  url: string,
  selector: string,
  providerId: number,
  fromCurrency: string,
  ...
  console.log(`=== AFRIEXAPP DIRECT SCRAPER ===`);
  ...
  try {
    ...
    console.log(`Retrieved ${html.length} characters from Afriexapp website`);
    
    // Parse the HTML
    const $ = cheerio.load(html);
    
    // Find elements matching the selector (div#converter-exchange-rate.converter-information__text-stm)
    ...
    elements.each((i, el) => {
      ...
      if (text.includes(fromCurrency) && text.includes(toCurrency)) {
        rateText = text;
        console.log(`Found rate text with both currencies: ${rateText}`);
      }
    });
    ...
    // Expected format: "1 GBP = 2131.65 NGN"
    const rateMatch = rateText.match(/(\d+(\.\d+)?)\s*${toCurrency}/i);
    if (!rateMatch || !rateMatch[1]) {
      // Try a more general pattern if the specific one fails
      const generalMatch = rateText.match(/(\d+(\.\d+)?)/);
      if (!generalMatch || !generalMatch[1]) {
        console.error(`Could not extract numeric rate from text: ${rateText}`);
        return false;
      }
      
      const rate = parseFloat(generalMatch[1]);
      if (isNaN(rate) || rate <= 0) {
        console.error(`Invalid rate value: ${generalMatch[1]}`);
        return false;
      }
      
      console.log(`Successfully extracted Afriexapp ${fromCurrency}-${toCurrency} rate: ${rate}`);
      ...
      console.log(`Successfully stored Afriexapp ${fromCurrency}-${toCurrency} rate: ${rate}`);
      return true;
    }
    
    const rate = parseFloat(rateMatch[1]);
    if (isNaN(rate) || rate <= 0) {
      console.error(`Invalid rate value: ${rateMatch[1]}`);
      return false;
    }
    ...
    console.error(`Error scraping Afriexapp rate:`, error);
    return false;
  }
}