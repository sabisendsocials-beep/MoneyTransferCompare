 ...
 * Usage: 
 *   npm run tsx initialize-providers.ts        # Add missing providers only
 *   npm run tsx initialize-providers.ts reset   # Delete all providers and re-add them
 */
...
/**
 * Initializes standard provider list
 */
async function initializeProviders() {
  const resetMode = process.argv.includes('reset');
  console.log(`Running provider initialization (reset mode: ${resetMode ? 'YES' : 'NO'})`);
  ...
  try {
    ...
    for (const provider of standardProviders) {
      if (!resetMode && existingNames.has(provider.name)) {
...
initializeProviders().then(() => {
  console.log('Provider initialization script completed');
  process.exit(0);
}).catch(err => {
  console.error('Error running provider initialization:', err);
  process.exit(1);
});